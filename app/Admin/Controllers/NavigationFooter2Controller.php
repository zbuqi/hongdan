...
class NavigationFooter2Controller extends AdminController
{
    ...
    {
        $footerLink2 = nav::where('type','=','footerLink2')->orderBy('sequence','desc')->get();
        return Form::make(new Navigation(), function (Form $form) use ($footerLink2){
            #获取当前时间
            $time = date('Y-m-d H:i:s', time());

            $form->text('title');
            $form->text('link','链接');
            $form->text('sequence','序号')->default($footerLink2[0]['sequence']+1);
